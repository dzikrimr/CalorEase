...
const Chatbot: React.FC<ChatbotProps> = ({ isOpen, onToggle, hasNotification = false }) => {
  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('bantuan')) {
      return 'Saya siap membantu Anda! Anda bisa bertanya tentang produk, layanan, atau informasi lainnya.';
    } else if (lowerMessage.includes('produk')) {
      return 'Kami memiliki berbagai produk berkualitas. Silakan kunjungi katalog kami atau tanyakan produk spesifik yang Anda cari.';
    } else if (lowerMessage.includes('kontak')) {
      return 'Anda bisa menghubungi kami melalui:\n📧 Email: support@calorease.com\n📞 Telepon: +62 21 1234567\n⏰ Jam kerja: 09.00 - 17.00 WIB';
    } else {
      const responses = [
        'Terima kasih atas pertanyaan Anda! Saya akan membantu Anda dengan senang hati.',
        'Itu pertanyaan yang menarik! Mari saya carikan informasi yang Anda butuhkan.',
        'Saya memahami kebutuhan Anda. Ada informasi lebih spesifik yang ingin Anda ketahui?',
        'Baik, saya akan membantu Anda menyelesaikan hal tersebut. Mohon tunggu sebentar.',
        'Pertanyaan yang bagus! Saya akan berikan penjelasan yang detail untuk Anda.'
      ];
