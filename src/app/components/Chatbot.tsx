...
const Chatbot: React.FC = () => {
  const { isOpen, toggleChatbot, recipe } = useChatbot();
  ...
  // Reset dimensions when chatbot is closed
  useEffect(() => {
    if (!isOpen) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div
          ref={chatbotRef}
          className="bg-white rounded-2xl shadow-2xl flex flex-col animate-slideInUp border border-teal-100 relative"
          style={{
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-white to-teal-50">
            ...
            {messages.map((message) => (
              <div
                key={message.id}
                className={`flex ${
                  message.sender === 'user' ? 'justify-end' : 'items-start gap-3'
                } animate-fadeIn`}
              >
                {message.sender === 'bot' && (
                  <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-teal-200 to-teal-300 rounded-full flex items-center justify-center text-sm">
