...
const RecipeDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchRecipeDetail = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/recipe-detail/${id}`, { signal: AbortSignal.timeout(10000) });
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(errorData.error || `Gagal mengambil resep: ${response.statusText}`);
        }
        const recipeData = await response.json();
        setCurrentRecipe(recipeData); // Use renamed setter
      } catch (err: any) {
        setError(err.name === 'TimeoutError' ? 'Permintaan waktu habis' : err.message);
  ...
  const handleCheckIngredients = () => router.push('/menu/food');

  const handleCheckSingleIngredient = (ingredientName: string) => {
    window.open(`https://www.tokopedia.com/search?q=${encodeURIComponent(ingredientName)}`, '_blank');
  };
  ...
  const handleAddToDaily = async () => {
    ...
    try {
      ...
      const currentCalories = userDoc.exists() ? userDoc.data().currentCalories || 0 : 0;
      const newCurrentCalories = currentCalories + recipe.nutrition.calories;
      await setDoc(userDocRef, { currentCalories: newCurrentCalories }, { merge: true });
    } catch (err: any) {
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-[#EDF8F7] to-[#D2EEE8]">
        ...
        <div className="flex-1 ml-70 flex items-center justify-center">
          <div className="text-center p-8">
            <div className="text-red-500 text-xl mb-4">⚠️ Error memuat resep</div>
            <p className="text-gray-600 mb-6">{error || 'Resep tidak ditemukan'}</p>
            <button
              onClick={() => router.back()}
      ...
      <div className="flex-1 flex flex-col ml-70 relative">
        <div className="flex flex-1 max-w-full">
            ...
            <div className="max-w-6xl mx-auto px-4 py-6">
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 border border-[#A5DDD1]/20">
                <div className="flex items-center justify-between">
                  ...
                  <h1 className="text-2xl font-bold text-[#198771]">Detail Resep</h1>
                ...
                <div className="lg:col-span-3 space-y-8">
                  <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-[#A5DDD1]/20">
                    ...
                    <div className="p-6">
                      <p
                        ...
                        dangerouslySetInnerHTML={{ __html: recipe.summary.replace(/<[^>]*>?/gm, '') }}
                      ></p>
                  ...
                  <div className="bg-white rounded-2xl shadow-lg p-6 border border-[#A5DDD1]/20">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h2 className="text-xl font-bold text-[#198771]">Bahan-Bahan</h2>
                    </div>
                      ...
                      {recipe.ingredients.map((ingredient, index) => (
                        ...
                        >
                          <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-sm flex-shrink-0">
                            <Image
                              src={ingredient.image || '/ingredients/default.jpg'}
                              alt={ingredient.name}
                              width={24}
                              height={24}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-[#198771] capitalize text-sm truncate">{ingredient.name}</p>
                          ...
                          <button
                            ...
                            className="bg-gradient-to-r from-[#1FA98D] to-[#4CBAA4] text-white px-2 py-1 rounded-lg hover:from-[#198771] hover:to-[#1FA98D] transition-all flex items-center gap-1 text-xs font-medium flex-shrink-0"
                          >
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 3H5.4m0 10a1 1 0 100 2 1 1 0 000-2zm10 0a1 1 0 100 2 1 1 0 000-2z" />
                            </svg>
                            Cek
                          </button>
                        </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-lg p-6 border border-[#A5DDD1]/20">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h2 className="text-xl font-bold text-[#198771]">Instruksi</h2>
                    </div>
                    <div className="space-y-4">
                      {recipe.instructions.map((instruction, index) => (
                        <div key={index} className="flex gap-4">
                          ...
                          <p className="text-[#198771] leading-relaxed flex-1 text-base">{instruction}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-lg p-6 border border-[#A5DDD1]/20">
                    <div className="space-y-4">
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-[#1FA98D] to-[#4CBAA4] text-white font-bold py-4 px-6 rounded-2xl hover:from-[#198771] hover:to-[#1FA98D] transition-all shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center gap-3"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 3H5.4m0 10a1 1 0 100 2 1 1 0 000-2zm10 0a1 1 0 100 2 1 1 0 000-2z" />
                        </svg>
                      ...
                      <button
                        ...
                        className={`w-full font-bold py-4 px-6 rounded-2xl transition-all shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center gap-3 ${
                          isLiked
                      ...
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path
                            ...
                            strokeWidth={2}
                            d={addedToDaily ? 'M5 13l4 4L19 7' : 'M12 6v6m0 0v6m0-6h6m-6 0H6'}
                          />
                        </svg>
                        {addedToDaily ? 'Ditambahkan ke Harian!' : 'Tambah ke Konsumsi Harian'}
                      </button>
          ...
          <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 hidden md:block">
            ...
            <div
              className={`bg-gradient-to-b from-[#1FA98D] to-[#4CBAA4] w-2 h-35 rounded-l-full shadow-lg cursor-pointer transition-all hover:w-3 hover:shadow-xl ${isSidebarVisible ? 'opacity-0' : 'opacity-100'}`}
              onMouseEnter={handleSidebarHover}
            ></div>
            <div
              className={`absolute right-0 top-1/2 -translate-y-1/2 transition-all duration-300 ease-in-out ${isSidebarVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}
            ...
            >
              <div className="w-30 bg-white/95 backdrop-blur-sm p-6 rounded-l-3xl shadow-2xl border-l border-t border-b border-[#A5DDD1]/30">
                <div className="space-y-6">
                  <div className="text-center">
                    <h3 className="text-lg font-bold text-[#198771] mb-4">Pelacak Kalori</h3>
                  </div>
                  <div className="space-y-6">
                    <div className="flex flex-col items-center">
                      <CalorieWaveTracker current={targetCalories} target={targetCalories} size={64} label="Target Kalori" />
                    </div>
                    <div className="flex flex-col items-center">
                      <CalorieWaveTracker current={currentCalories} target={targetCalories} size={64} label="Kalori Hari Ini" />
                    </div>
                    <div className="flex flex-col items-center">
                      <CalorieWaveTracker
                  ...
                  <div className="pt-4 border-t border-[#A5DDD1]/30">
                    <div className="text-center text-sm text-[#198771]/70">
                      <p>Progres: {Math.round((currentCalories / targetCalories) * 100)}%</p>
                    </div>
                  </div>
                </div>
              </div>
